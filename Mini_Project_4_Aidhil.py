...
class LinkedList:
    ...
    def fibonacci_search(self, key, by='nama'):
        arr = []
        current = self.head
        while current:
            if by == 'nama':
                ...
                arr.append(current.warna)
            current = current.next

        result_index = fibonacci_search(arr, key)
    ...
    while True:
        ...
        if pilihan == "1":
            ...
            if search_choice == "1":
                ...
                result = daftar_ikan.fibonacci_search(keyword, by='nama')
                if result:
                ...
                result = daftar_ikan.fibonacci_search(keyword, by='warna')
                if result:
                    print("Ikan ditemukan:")
                    print("Nama:", result.data)
                    print("Berat:", result.berat)
